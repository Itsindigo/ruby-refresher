...
def get_first_half_of_string(string)
  string = string[0..((string.length - 1) / 2)]
end
...
def total_of_array(array)
  array.inject {|sum, total| total + sum}
end
...
def average_of_array(array)
  total = array.inject{|sum, total| total + sum}.to_f
...
def add_together_keys_and_values(hash)
  hash.to_a.flatten.inject {|sum, total| total += sum}
end
...
def titleize_a_string(string)
  array = string.split(" ")
  array.each do |w|
    w.capitalize! unless w == "and" || w == "the" || w == "a"
  end
  array[0].capitalize!
  return array.join(" ")
end
